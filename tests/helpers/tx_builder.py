...
def is_tokens_equal(t1: Token, t2: Token) -> bool:
    return bytes(t1.token_id).hex() == bytes(t2.token_id).hex() and t1.amount == t2.amount

def to_data_input(data_input: DataInput):
    return str(data_input.box_id)
...
class ErgoToken:
    def __init__(self, token_id: str, amount: int):
        self.id = token_id
        self.amount = amount
...
class AppTx:
    def __init__(self,
                 inputs: list[UnsignedBox],
                 data_inputs: list[str],
                 outputs: list[ErgoBoxCandidate],
                 distinct_token_list: list[TokenId],
                 change_map: ErgoChangeMap | None):
        self.inputs = inputs
        self.data_inputs = data_inputs
        self.outputs = outputs
        self.distinct_token_ids = distinct_token_list
        self.change_map = change_map

class AttestedTransaction:
    def __init__(self,
                 inputs: list[AttestedBox],
                 data_inputs: list[str],
                 outputs: list[ErgoBoxCandidate],
                 distinct_token_list: list[TokenId],
                 change_map: ErgoChangeMap | None):
...
class ErgoTxBuilder:
    ...
    def input(self,
              ...
              index: int,
              amount: int,
              tokens: list[ErgoToken] | None = None):
        stokens: list[Token] | None = None
        if tokens is not None:
            stokens = []
            for stoken in tokens:
                token_id = TokenId(stoken.id)
                stokens.append(Token(token_id, stoken.amount))

        ergo_box = create_ergo_box(extended_address.address.to_str(extended_address.network),
                                   tx_id, index,
                                   amount,
                                   stokens)
    ...
    def box_input(self, ergo_box: ErgoBox, path: DerivationPath):
        ...
        box = to_unsigned_box(ergo_box, context_extension, str(path))
    ...
    def output(self,
               ...
               value: int,
               send_tokens: list[ErgoToken] | None = None,
               mint_token_amount: int | None = None):
        stokens: list[Token] | None = None
        ...
        mint_token = None
        if mint_token_amount is not None:
            token_id = TokenId(str(self.inputs[0].ergo.box_id))
            mint_token = Token(token_id, mint_token_amount)
            self.minting_outputs.append(mint_token)
        else:
            self.minting_outputs.append(None)

        if mint_token is not None:
            if stokens is None:
                stokens = []
    ...
    def burn(self, tokens: list[ErgoToken]):
        for token in tokens:
            ...
            self.burning_tokens.append(Token(token_id, token.amount))
    ...
    def change(self, extendedAddress: ExtendedAddress):
        self.change_map = ErgoChangeMap(extendedAddress.address.to_str(extendedAddress.network),
                                        str(extendedAddress.path))
    ...
    def build(self):
        ...
        for idx, output in enumerate(self.outputs):
            ...
            for tkn in output.tokens:
                if (minting is not None and not is_tokens_equal(tkn, minting)) or minting is None:
        ...
        inputs:list[ErgoBox] = []
        for input_elt in self.inputs:
            inputs.append(input_elt.ergo)
        ...
        for input_elt in self.data_inputs:
            data_inputs.append(input_elt)
        ...
        for inp in ergo_tx.inputs:
            for linp in self.inputs:
                if str(inp.box_id) == str(linp.ergo.box_id):

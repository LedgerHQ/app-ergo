...
class ErgoCommandSender:
    ...
    def derive_address(self,
                       ...
                       path: str,
                       token: int | None = None,
                       show: bool = False) -> Generator[None, None, None]:
        ...
        with self.backend.exchange_async(cla = CLA,
                                     ...
                                     p2   = P2.P2_WITHOUT_TOKEN if token is None else P2.P2_WITH_TOKEN,
    ...
    def ext_pub_key(self, path: str, token: int | None = None) -> Generator[None, None, None]:
        ...
        with self.backend.exchange_async(cla = CLA,
                                     ...
                                     p1   = P1.P1_PK_WITHOUT_TOKEN if token is None else P1.P1_PK_WITH_TOKEN,
                                     p2   = P2.P2_ZERO,
                                     data = data) as response:
            yield response
    ...
    def attest_send_tokens(self, tokens: list[Token], session_id: int) -> int:
        ...
        for chunk in chunks:
            ...
            for token in chunk:
                data.write_bytes(bytes(token.token_id)).write_uint64(token.amount)
        ...
        return latest.data[0] if latest is not None and len(latest.data) > 0 else 0
    ...
    def stx_send_distinct_tokens_ids(self, session_id: int, token_ids: list[TokenId]):
        tkns = [bytes(i) for i in token_ids]
    ...
    def stx_send_inputs(self, session_id: int, list_inputs: list[AttestedBox]):
        for input_elt in list_inputs:
            for frame in input_elt.frames:
                self.backend.exchange(cla = CLA,
                                     ins  = InsType.SIGN_TX,
            ...
            if len(input_elt.box.extension) > 0 and \
                not (len(input_elt.box.extension) == 1 and \
                     input_elt.box.extension[0] == 0x00):
    ...
    def stx_add_output_box_tokens(self, session_id: int, tokens: list[Token], distinct_token_ids: list[str]):
        ...
        for chunk in chunks:
            ...
            for token in chunk:
                writer.write_uint32(distinct_token_ids.index(bytes(token.token_id).hex()))
    ...
    def stx_send_outputs(self,
                         session_id: int,
                         boxes: list[ErgoBoxCandidate],
                         change_map: ErgoChangeMap,
                         distinct_token_ids: list[TokenId]):
        distinct_token_ids_str = [bytes(i).hex() for i in distinct_token_ids]
        for box in boxes:
            writer = ErgoWriter(ADD_OUTPUT_HEADER_SIZE)
            writer.write_uint64(box.value)
            writer.write_uint32(len(bytes(box.ergo_tree)))
            ...
            tree: str = bytes(box.ergo_tree).hex()
            tree_addr = None
            try:
                tree_addr = Address(bytes(box.ergo_tree)).to_str()
            except:
                pass
            ...
            if tree == MINER_FEE_TREE:
                ...
                self.stx_add_output_box_ergo_tree(session_id, bytes(box.ergo_tree))
            ...
            if len(box.additional_registers) > 0:
                self.stx_add_output_box_registers(session_id, bytes(box.additional_registers))
    ...
    def sign_tx(self,
                ...
                sign_path: str,
                network: int,
                token: int | None = None) -> Generator[bytes | None, None, None]:
    ...
    def sign_tx_flow(self,
                     tx: AppTx,
                     network: int,
                     token: int | None = None) -> Generator[StxState | list[bytes], None, None]:
        if len(tx.inputs) == 0:
            raise ValueError("inputs is empty")

        attested_inputs:list[AttestedBox] = []

        for input_elt in tx.inputs:
            for nb in self.attest_input(input_elt, None):
                if nb is not None:
                    nb.set_extension(input_elt.extension)
        ...
        attested_tx = AttestedTransaction(attested_inputs,
                                          tx.data_inputs,
                                          tx.outputs,
                                          tx.distinct_token_ids,
                                          tx.change_map)
        ...
        for input_elt in tx.inputs:
            sign_bytes.append(signatures[input_elt.sign_path])

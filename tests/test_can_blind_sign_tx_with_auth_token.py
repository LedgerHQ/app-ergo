...
def test_can_blind_sign_tx_with_auth_token(device: Device,
    ...
    for nb in client.sign_tx_flow(tx.app_tx, int(NETWORK), AUTH_TOKEN):
        ...
        if nb == StxState.ATTEST:
            ...
            scenario_navigator.review_approve_with_warning(
                custom_screen_text="Sign transaction" if device.is_nano else None)
